...
Example 2:
  ...
  Explanation: The longest possible good subarray is [1,2] since the values 1 and 2 occur at most
    once in this subarray. Note that the subarray [2,1] is also good. It can be shown that there
    are no good subarrays with length more than 2.
...
class Solution2958 : public ISolution {
public:
    Solution2958();
    ...
    {
        return ISolution::buildPrefix() + "2958]";
    }

private:
    // Runtime: 0 ms / Memory: 6.73 MB
    int maxSubarrayLength(std::vector<int>& nums, const int k);
    bool isSubarrayGood(std::vector<int>& nums, int i, int j, int k);
};